...
| 构建依赖 | [dependencies.lock.json](../dependencies.lock.json)、[toolchain.py](../scripts/toolchain.py) |

协议适配与 SSE 解码统一在 [OpenAI 客户端](../dependencies/openai/client.norm)，支持 Responses 与 Chat Completions。Agent 使用统一的 Response 和 FunctionCall；Chat 的消息、工具增量和 reasoning_content 由客户端转换。这些上下文只存在于本次调用内，不落盘，不使用服务端历史 ID。工具只从正式协议字段取得，串行执行。
